...
prepare_422() {
    ...
    print_and_run \
        ffmpeg -f lavfi -i "mandelbrot=size=${1}" \
            -vframes "${2}" -pix_fmt yuv422p -f yuv4mpegpipe \
            "${yuvfile}"
}
...
valgrind_test_422() {
    ...
    print_and_run \
        ../libtool execute $valgrind \
            ../src/kvazaar -i "${yuvfile}" --input-format=P422 "--input-res=${dimensions}" -o "${hevcfile}" "$@"
...
prepare_444() {
    ...
    print_and_run \
        ffmpeg -f lavfi -i "mandelbrot=size=${1}" \
            -vframes "${2}" -pix_fmt yuv444p -f yuv4mpegpipe \
            "${yuvfile}"
}
...
valgrind_test_444() {
    ...
    print_and_run \
        ../libtool execute $valgrind \
            ../src/kvazaar -i "${yuvfile}" --input-format=P444 "--input-res=${dimensions}" -o "${hevcfile}" "$@"
